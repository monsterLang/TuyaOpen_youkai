...
// RINGBUF size
#define PCM_BUF_SIZE (uint32_t)(MIC_RECORD_DURATION_MS * MIC_SAMPLE_RATE * MIC_SAMPLE_BITS * MIC_CHANNEL / 8 / 1000)
// 10ms PCM data size
#define PCM_FRAME_SIZE (uint32_t)(10 * MIC_SAMPLE_RATE * MIC_SAMPLE_BITS * MIC_CHANNEL / 8 / 1000)

#define STATUS_WAIT 0
#define STATUS_START_RECORD 1
#define STATUS_END_RECORD 2
...
static BOOL_T audio_trigger_pin_is_pressed(void)
{
    ...
    if (level == TUYA_GPIO_LEVEL_LOW){
        ...
        flag_status += 1;
        if(flag_status == STATUS_INVALID)
            flag_status = STATUS_WAIT;
...
static void app_recorder_thread(void *arg)
{
    ...
    PR_DEBUG("%s Test02 -- mic\n Press AUDIO_KEY twice to start recording.", __func__);

    for (;;) {
        if(flag_status == STATUS_START_RECORD)
            app_mic_record();

        if (FALSE == audio_trigger_pin_is_pressed() && flag_status == STATUS_END_RECORD) {
        ...
        // Start recording
        if ((FALSE == sg_recorder_ctx.recording) && flag_status == STATUS_START_RECORD) {
#if (RECORDER_FILE_SOURCE == USE_INTERNAL_FLASH) || (RECORDER_FILE_SOURCE == USE_SD_CARD)
...
static OPERATE_RET app_fs_init(void)
{
    ...
    if (rt != OPRT_OK) {
        PR_ERR("mount sd card failed, please retry after format [fat]");
        return rt;
    }
